...
export const ExchangeHistory = () => {
  ...
  const { exchangeHistory, getTransactionHistory } = useAppContext();
  ...
  useEffect(() => {
    filterHistory();
  }, [exchangeHistory, keywords, searchCoin, searchType]);
  ...
  const fetchData = async ({ isShowLoading }) => {
    ...
    const params = { c: 'all', t: 'buysell', l: 1000 };
  ...
  const filterHistory = () => {
    ...
    const filtered = multiFilter({ arr: exchangeHistory, filters: filters });
